...
from sam2.modeling.sam.prompt_encoder import PromptEncoder as SamPromptEncoder
...
class MaskProcessor(nn.Module):
    def __init__(self, hidden_dim, image_size, reduction, **kwargs):
        ...
        self.prompt_encoder_sam = SamPromptEncoder(
    ...
    def load_sam2_weights(self, checkpoint):
        """Load mask decoder / prompt encoder weights from a SAM2 state dict."""
        state_dict = {
            k.replace("mask_decoder.", "").replace("sam_", ""): v
            for k, v in checkpoint.items()
            if "mask_decoder" in k
        }
        self.mask_decoder.load_state_dict(state_dict)
        state_dict = {
            k.replace("prompt_encoder.", "").replace("sam_", ""): v
            for k, v in checkpoint.items()
            if "prompt_encoder" in k
        }
        self.prompt_encoder_sam.load_state_dict(state_dict)
        state_dict = {
            k: v for k, v in checkpoint.items() if "no_mem_embed" in k
        }
    ...
    def forward(self, features_orig, outputs):
        ...
        for img_idx in range(len(outputs)):
            ...
            if len((outputs[img_idx]['pred_boxes'][0])) > 800:
                ...
                batch_masks.append([])
                batch_bboxes.append(
                    outputs[img_idx]['pred_boxes'].squeeze() * self.image_size
                )
                batch_iou.append(outputs[img_idx]['box_v'])
                continue
            features = {
                'vision_features': features_orig['vision_features'][
                    img_idx
                ].unsqueeze(0),
                'vision_pos_enc': [
                    x[img_idx].unsqueeze(0)
                    for x in features_orig['vision_pos_enc']
                ],
                'backbone_fpn': [
                    x[img_idx].unsqueeze(0)
                    for x in features_orig['backbone_fpn']
                ],
            }
            ...
            masks_ = []
            for box_i in range(
                step, len(outputs[img_idx]['pred_boxes'][0]) + step, step
            ):
                box = (
                    outputs[img_idx]['pred_boxes'][0][(box_i - step) : box_i]
                    * self.image_size
                )
                box_coords = box.reshape(-1, 2, 2)
                box_labels = torch.tensor(
                    [[2, 3]], dtype=torch.int, device=box.device
                )
                ...
                corrected_bboxes = torch.zeros(
                    (masks.shape[0], 4), dtype=torch.float
                )
                masks = masks[:, 2]
