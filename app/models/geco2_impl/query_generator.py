...
from .attention import PrototypeAttentionBlock
...
class C_base(nn.Module):
        ...
        for _ in range(num_prototype_attn_steps):
            self.prototype_attention.append(
                PrototypeAttentionBlock(
                    embedding_dim=transformer_dim, num_heads=8
                )
            )
    ...
    def forward(
        ...
        if (
            not hasattr(self, "reference_points")
            or self.spatial_shapes.device != image_embeddings.device
        ):
            self.spatial_shapes = self.spatial_shapes.to(
                image_embeddings.device
            )
            self.spatial_shapes1 = self.spatial_shapes1.to(
                image_embeddings.device
            )
            self.spatial_shapes2 = self.spatial_shapes2.to(
                image_embeddings.device
            )
            self.level_start_index = self.level_start_index.to(
                image_embeddings.device
            )
            self.level_start_index1 = self.level_start_index1.to(
                image_embeddings.device
            )
            self.level_start_index2 = self.level_start_index2.to(
                image_embeddings.device
            )
        ...
        image_pe = torch.repeat_interleave(
            image_pe, image_embeddings.shape[0], dim=0
        )
        ...
        for layer in self.prototype_attention_l1:
            hq_features_l1 = layer(
                image_f=hq_features_l1, prototypes=hq_prototypes[0]
            )

        for layer in self.prototype_attention_l2:
            hq_features_l2 = layer(
                image_f=hq_features_l2, prototypes=hq_prototypes[1]
            )
        ...
        hq_features_l2 = hq_features_l2.transpose(1, 2).view(
            b, c, h * 2, w * 2
        )
        hq_features_l1 = hq_features_l1.transpose(1, 2).view(
            b, c, h * 4, w * 4
        )
